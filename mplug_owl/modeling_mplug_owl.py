...
class StreamingLlama(LlamaForCausalLM):
...
class mPLUG_OwlForConditionalGenerationStream(mPLUG_OwlForConditionalGeneration):
    def __init__(self, config: mPLUG_OwlConfig):
        super().__init__(config)
        self.language_model = StreamingLlama(config=config.text_config)
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        """
        Overrides `generate` function to be able to use the model as a conditional generator and 
        implements token streaming.
        ...
        for output in self.language_model.generate(
            inputs_embeds=inputs_embeds,
            # input_ids=input_ids,
            attention_mask=attention_mask,
            **generate_kwargs,
